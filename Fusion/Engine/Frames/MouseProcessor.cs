...
namespace Fusion.Engine.Frames {
	...
	class MouseProcessor {
	    ...
	    private static int SysInfoDragWidth
        {
            get
            {
                return System.Windows.Forms.SystemInformation.DragSize.Width;
            }
        }

	    private static int SysInfoDragHeight
        {
            get
            {
                return System.Windows.Forms.SystemInformation.DragSize.Height;
            }
        }
		...
		public void Update ( Frame root )
		{
			...
			if ( mousePoint!=oldMousePoint ) {
			    ...
			    if (!IsPointWithinDragArea(mousePoint, oldMousePoint))
			    {			        
                    if (heldFrame != null)
                    {
                        dragFrame = true;
                        heldFrame.OnDrag(dx, dy);
                    }
                }
        ...
        static bool IsPointWithinDragArea(Point a, Point b)
        {
            var dx = Math.Abs(a.X - b.X);
            var dy = Math.Abs(a.Y - b.Y);
            return (dx < SysInfoDragWidth && dy < SysInfoDragHeight);
        }
