...
namespace Fusion.Engine.Graphics {
	public class SpriteDrawer : SpriteLayer {



        public TargetTexture Output;
	    ...
	    public SpriteDrawer(RenderSystem rs, int capacity, int left, int top, int width, int height) : base(rs, capacity)
		{
            Output = new TargetTexture(rs, width, height, TargetFormat.HighDynamicRange);
		}
		...
		{
		    DepthStencilSurface dsOld;
		    RenderTargetSurface[] rtOld;
		    rs.Device.GetTargets(out dsOld, out rtOld);            
            rs.Device.SetTargets(null, Output.RenderTarget);
			if (spriteCount==0) {
				return;
			}
            ...
            rs.Device.SetTargets(dsOld, rtOld);       
		}
	    ...
	    public void DrawFreeUV(Texture srv, Vector2 tl, Vector2 tr, Vector2 bl, Vector2 br, Color color, Vector2 uvtl,
	        Vector2 uvtr, Vector2 uvbl, Vector2 uvbr, int clipRectIndex = 0)
	    {
            var c = color;
            PushQuad(srv,
                     new SpriteVertex(tl.X, tl.Y, 0, c, uvtl.X, uvtl.Y, clipRectIndex),
                     new SpriteVertex(tr.X, tr.Y, 0, c, uvtr.X, uvtr.Y, clipRectIndex),                     
                     new SpriteVertex(br.X, br.Y, 0, c, uvbr.X, uvbr.Y, clipRectIndex), 
                     new SpriteVertex(bl.X, bl.Y, 0, c, uvbl.X, uvbl.Y, clipRectIndex));
        }
