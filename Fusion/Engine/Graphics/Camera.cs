		...
		public void SetupCamera ( Matrix viewMatrix, float height, float width, float near, float far, float convergence, float separation )
		{
			...
			if (Game.Instance.GraphicsDevice.Display is OculusRiftDisplay) {
				...
				if (OculusRiftSensors.LeftEye != null && OculusRiftSensors.RightEye != null) {
					...
					//var camMat = Matrix.Invert(viewMatrix);
					//
					//var originPos = camMat.TranslationVector;
					//var originRot = Quaternion.RotationMatrix(camMat);
					//
					//
					//Vector3 posL = originPos + Vector3.Transform(leftEyePos, originRot);
					//var 	rotL = originRot * leftRot;
					//
					//Vector3 finalUpL		= Vector3.Transform(new Vector3(0, 1, 0), rotL);
					//Vector3 finalForwardL	= Vector3.Transform(new Vector3(0, 0, -1), rotL);
					//Matrix oculusViewMatL = Matrix.LookAtRH(posL, posL + finalForwardL, finalUpL);
					//
					//Vector3 posR = originPos + Vector3.Transform(rightEyePos, originRot);
					//var 	rotR = originRot * rightRot;
					//
					//Vector3 finalUpR		= Vector3.Transform(new Vector3(0, 1, 0), rotR);
					//Vector3 finalForwardR	= Vector3.Transform(new Vector3(0, 0, -1), rotR);
					//Matrix oculusViewMatR = Matrix.LookAtRH(posR, posR + finalForwardR, finalUpR);
					//
					//this.viewMatrixL = oculusViewMatL;
					//this.viewMatrixR = oculusViewMatR;

					viewMatrixL = viewMatrix * Matrix.Translation(leftEyePos)	* Matrix.RotationQuaternion(leftRot)	;
					viewMatrixR = viewMatrix * Matrix.Translation(rightEyePos)	* Matrix.RotationQuaternion(rightRot)	;
