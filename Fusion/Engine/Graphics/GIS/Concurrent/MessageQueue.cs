...
namespace Fusion.Engine.Graphics.GIS.Concurrent
{
	...
	public partial class MessageQueue
	{
		...
		public void StartPriorityInAnotherThread()
		{
			lock (queue) {
				if (state != State.Stopped)
					throw new InvalidOperationException("The MessageQueue is already running.");
			...
			Task.Factory.StartNew(PriorityRun);
		}
		...
		public void Post(Action<WorkRequest> callback, object userData, int priority = 0)
		{
			lock (queue) {
				queue.Add(new WorkRequest(callback, userData, null, priority) {Flags = WorkRequest.InfoFlags.FillProcessQueue | WorkRequest.InfoFlags.FillRWQueue});
				Monitor.Pulse(queue);
			}
		}


		public void Post(WorkRequest request)
		{
			lock (queue) {
				queue.Add(request);
				Monitor.Pulse(queue);
