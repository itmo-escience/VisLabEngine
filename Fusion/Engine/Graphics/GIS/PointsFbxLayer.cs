...
{
	public class PointsFbxLayer : Gis.GisLayer
	{
		Ubershader      shader;
		...
		public struct FbxPoint
		{
			public double Lon;
			public double Lat;
			public Color4 Color;
			public Vector4 AngleSizeHeightX;
		}

		StructuredBuffer points;

		public FbxPoint[]	PointsCpu	{ get; protected set; }
		...
		public PointsFbxLayer(Game engine, string fbxFileName, int maxPointsCount) : base(engine)
		{
			var scene	= engine.Content.Load<Scene>(fbxFileName);
			FbxMesh		= scene.Meshes.First();

			PointsCpu = new FbxPoint[maxPointsCount];

			shader	= engine.Content.Load<Ubershader>("globe.Fbx.hlsl");
			...
			points = new StructuredBuffer(engine.GraphicsDevice, typeof(FbxPoint), maxPointsCount, StructuredBufferFlags.None);
		}
