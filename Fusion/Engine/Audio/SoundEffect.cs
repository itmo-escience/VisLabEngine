...
namespace Fusion.Engine.Audio
{
    public sealed class SoundEffect : IDisposable
    {
	    ...
	    public SoundEffect(SoundSystem device, WaveFormat format, DataStream stream)
	    {
		    this.device = device;
		    Initialize(format, stream);
	    }


	    private void Initialize(WaveFormat format, DataStream stream)
	    {
		    _format = format;
	    ...
	    public static SoundEffect CreateFromFile(Game game, string fileName)
	    {
			var audioStream = new SoundStream(File.OpenRead(fileName));
		    var dataStream = audioStream.ToDataStream();
		    var effect = new SoundEffect(game.SoundSystem, audioStream.Format, dataStream);
	    ...
	    {
		    if (_playingInstances != null) {
				_playingInstances.ForEach(x=>x.Stop(true));
			}
	    }
