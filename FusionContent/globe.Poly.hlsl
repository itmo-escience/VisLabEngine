...
VS_OUTPUT VSMain ( VS_INPUT v )
{
	...
	double3 cPos	= SphericalToDecart(double2(lon, lat), 6378.137 + double(v.Tex1.w));
...
float4 PSMain ( VS_OUTPUT input ) : SV_Target
{
		...
		return float4(color.rgb, color.a * HeatStage.Data.z);
			...
			float3 color = input.Color.xyz * DiffuseMap.Sample(Sampler, float2(HeatStage.Data.x, 0.5f)).xyz;
