...
inline float3 SpringForce(float3 pos1, float3 pos2, float linkLen, float strength)
{
	...
	lenDif = clamp(lenDif/linkLen, -1000.0f, 1000.0f);
...
void CSMain( uint3 groupID : SV_GroupID, uint groupIndex : SV_GroupIndex )
{
	...
	if (!CheckParticle(p) || p.Mass <= 0) return;	
	...
	p.Position.xyz += vel * deltaTime * 2;// mul( p.Force, Params.DeltaTime * 100 );
