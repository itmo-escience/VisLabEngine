...
inline float4 pairBodyForce( float4 thisPos, float4 otherPos ) // 4th component is charge
{
	...
	return  float4( R , energy)* invRCubed; // we write energy into the 4th component
}
...
float4 tileForce( float4 position, uint threadId, float radius )
{
...
float4 calcRepulsionForce( float4 position, uint3 groupThreadID )
{
	...
	for ( uint i = 0; i < Params.MaxParticles; i+= BLOCK_SIZE, tile += 1 )
	{
		...
		force += tileForce( position, groupThreadID.x, p.Size0) ;
...
#ifdef EULER
	//force = mul( calcRepulsionForce( pos, groupThreadID ),  pos.w ) ; // we multiply by all the constants here once
#ifdef LINKS
	force += calcLinksForce ( pos, id, p.LinksPtr, p.LinksCount ) / 100000;
#endif // LINKS
#endif // EULER
...
)
{
	...
	if (id < Params.MaxParticles) {
		...
		p.Position.xyz += mul( p.Force, Params.DeltaTime * 100 );
		particleRWBuffer[ id ] = p;
	}
}
#endif // MOVE
