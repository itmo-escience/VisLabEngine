...
- [x] Назначить всем необходимым свойствам NotifyPropertyChanged
...
- [x] Начинает тормозить при большом количестве элементов на экране (Edit-режим отрисовки границ всех компонентов)
...
- [x] При назначении родителя тперь обновляет перень свойств в соответствии с новым слотом
...
- [ ] При включении свойства AutoSize у Label из False в True нет автоматического переизмерения (AutoSize срабатывает только после изменения текста)
...
- [ ] Падает при вставке контейнеров с детьми (и при загрузке из файла)

- [x] Обновить стили в редакторе
### Тестирование на котиках 2
- [ ] Не работают прилипания внутри контейнера
...
- [ ] Режим переименования при двойном клике в тривью
- [ ] Не отображаются ширина/высота сцены в верхнем меню
- [ ] Нигде не отображается тип выделенного компонента
- [ ] Перетаскивание в AnchorBox
- [ ] Падает при удалении двух и более элементов одновременно (тривью не успевает отловить изменения в коллекции (синхронизация))
- [ ] После сериализации компоненты схлопываются (под влиянием WPF)
