...
- [ ] Перемещение по сцене при зажатом колесе мыши (инструмент "Рука")
...
- [x] Дублирование в дереве элементов первого непосредственно брошенного на контейнер элемента (вводит в заблуждение)
- [x] При переключении сцены странно отрисовывает сцену (маленькую)
- [x] Запретить выбор детей текущего выбранного элемента при одиночном клике без Shift'а (ожидается перемещение элемента, а вместо него перемещается ребенок)
- [x] Новые компоненты добавляются на сцену
- [x] При назначении родителя теперь обновляет перечень свойств в соответствии с новым слотом
- [x] Не видит компоненты под мышью в глубину, так как возвращает перетаскиваемый элемент
- [x] При включении свойства AutoSize у Label из False в True нет автоматического переизмерения (AutoSize срабатывает только после изменения текста)
- [x] Падает при вставке контейнеров с детьми (и при загрузке из файла)
- [x] Назначить всем необходимым свойствам NotifyPropertyChanged (реализовано Fody)
- [x] Не работают прилипания внутри контейнера
- [x] Вкл/откл режим дебаг в UIManager
- [x] Сделать приватными сеттеры для X, Y в AnchorBoxSlot
- [x] Падает при дропе нового компонента сразу в дерево (м.б. должно создавать там компонент)
- [x] Нигде не отображается тип выделенного компонента
- [x] Перетаскивание в AnchorBox
- [x] Падает при удалении двух и более элементов одновременно (тривью не успевает отловить изменения в коллекции (синхронизация))
- [x] После сериализации компоненты схлопываются (под влиянием WPF)
- [x] Скрыть ненужные свойства в окне свойств компонента и определить корректное отображение для нестандартных структур
  - [x] Slot
  - [x] TextFormat
  - [x] Fixators
  - [x] Style
- [x] Добавить логику многократного клика по элементам (для контейнеров) как в Figma
- [x] Выделение рамкой должно захватывать элементы при пересечении, а не при полном нахождении элементов в ней
- [x] Элементы при бросании на компонент делать детьми только при начале перетаскивания с нажатой клавишей "Alt" (в противном случае элемент остается в том же родителе)
- [x] Выделение рамкой детей сделать на "Ctrl"
- [x] BoundingBox не отображает глобальные координаты
- [x] Избавиться от назначения свойств компоненту через компоненты WPF (менять нужно напрямую как через окно свойств компонента)
- [x] Сделать Reparenting при перемещении мышью
- [x] VerticalBox иногда схлапывает своих детей при назначении
- [x] Обновить стили в редакторе
- [x] Невозможность выделить рамкой дочерние элементы контейнера (выбирается и мгновенно перетаскивается контейнер) (?) (отложить до обсуждения слотов)
...
- [ ] починить treeview:
    ...
    - [x] При добавлении в пустой контейнер при помощи перетаскивания из палитры дублируется запись в treeview
...
- [ ] Контроллеры
  - [x] Отсутствие контроля z-order-а у слотов.
  - [ ] Отсутствие возможности делать switch по состояниям. (не получится)
...
- [ ] Прозрачность изображений работает некорректно. (отложено)
- [ ] Падение при закрытии редактора во FusionRocket у Влада. 
...
- [ ] Вернуть глобальный угол (отложено)

- [ ] VerticalBox тормозит при перемещении по сцене
- [ ] Элементы копируются внутрь родителя текущего выбранного элемента (без выбора кидаются на сцену)

### Тестирование на котиках 2

- [x] Перетаскивание с дерева компонентов объектов на сцену - запретить/исправить
- [x] Снимать выделение при клике по пустому месту (в том числе по серому фону вокруг)
- [x] Скролл при зажатом колесе мыши 
- [x] Неудобно зумить к углу при зуме меньше единицы (доделать)

- [ ] Режим переименования при двойном клике в тривью (отложить)
- [ ] Не отображаются ширина/высота сцены в верхнем меню (отложить)
- [ ] После переизмерения компонента его перемещение начинает тормозить (просто БЕЗБОЖНО тормозит)
- [ ] Запретить изъятие слотов из контроллеров

- [ ] Исправить интерференцию глубинного клика и выбора детей рамкой
- [ ] Исправить выбор рамкой при зажатом Shift
- [ ] Добавить команду PropertyChange для вложенных свойств
- [ ] Починить Ctrl+Z/Ctrl+Y с учетом логики слотов
