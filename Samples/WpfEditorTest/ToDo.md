...
- [ ] Курсор сбрасывается в окне деталей компонента
- [ ] В диалаоговом окне переизмерения сцены добавить Enter и Esc для срабатывания "ОК" и "Отмена" соответственно
- [ ] При сохранении новой сцены в файл имя вкладки не меняется (дожно обновляться как в заголовке окна редактора)
- [ ] При полном откате стека изменений сцена все еще считается "грязной"(?)
- [ ] Задизейблить кнопку "File"->"Save template", если нет выделенных элементов
- [ ] Для каждой сцены хранить свой стек команд Undo/Redo (некорректная работа при переключении между сценами)
- [ ] Сделать центром Zoom'а положение мыши на сцене
...
- [ ] Скрыть ненужные свойства в окне свойст компонент аи определить корректное отображение для нестандартных структур