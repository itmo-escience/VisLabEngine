...
- Прикрепление к проектам
  ...
  - [ ] Добавить путь к директории со сценами в ICustomazableUI и предлагать по умолчанию при сохранениии/загрузке сцены.
---
### DirectD2D
...
- [ ] Фрейм невозможно перемещать при помощи кнопок (кнопки перехватываются ScrollView)
...
- [ ] починить treeview: нет автоматического выделения добавленных элементов
- [x] Переизмернеие повернутого фрейма
  - [x] Учитывать поворот при переизмерении 
  - [ ] Переизмерение группы, в которых разные фреймы повернуты по разному
...
- [ ] После изменения размера сцены прекращается ее отрисовка.
- [ ] GlobalAngle у компонента считается неверно.
- [ ] Найти все возможные утечки памяти (dispose)
- [ ] При бросании фрейма из повернутого контейнера в повернутый контейнер, он сползает.
- [ ] Относительный путь для картинки.
- [ ] Отключать прилипание при зажатом Alt.
- [ ] Прилипание к фреймам сделать по BoundingBox.
- [ ] При попытке поменять размер сцены при переключениях между вкладками периодически изображение виснет
- [ ] Окно консоли
  ...
  - [ ] Возврат к старой команде через клавиши со стрелками.
