...
namespace FusionUI.UI.Factories
{
    public class ColorPickerFactory
    {
        public static UIContainer<Slider, Slider, Slider> ColorPickerHolder(FrameProcessor ui, float OffsetX, float OffsetY,
            ...
            Action<Color> changeAction, Color initValue, out ScalableFrame colorHolder, bool needAlpha = true)
        {
            UIContainer<Slider, Slider, Slider> holder = new UIContainer<Slider, Slider, Slider>(ui, parent.UnitPaddingLeft, 0, parent.UnitWidth - parent.UnitPaddingLeft - parent.UnitPaddingRight, UIConfig.UnitSliderLabelHeight + UIConfig.UnitSliderObjectHeight*(needAlpha ? 4 : 3) + OffsetY, "", Color.Zero);
            ...
            if (needAlpha) alphaSlider = new Slider(ui, OffsetX, OffsetY + UIConfig.UnitSliderLabelHeight + UIConfig.UnitSliderObjectHeight * 3, holder.UnitWidth - UIConfig.UnitColorPickerSampleWidth - 2 * OffsetX - UIConfig.UnitColorPickerSampleOffset, UIConfig.UnitSliderObjectHeight, "", Color.Zero)
            {
                backColorForSlider = ColorConstant.BackColorForSlider,
                ForeColor = ColorConstant.ForeColor,
                HoverColor = ColorConstant.HoverColor,
                ...
                MinValue = 0,
                MaxValue = 1,
                CurrentValue = initValue.A * (1.0f / 255),
                UnitSliderWidth = 0.5f,
                UnitWidthControlElement = 4,
                UnitHeightControlElement = UIConfig.UnitSliderObjectHeight,
                Image = ui.Game.Content.Load<DiscTexture>(@"UI-new\fv-icons_slider"),
            ...
            blueSlider.OnChange += f => {
                sample.BackColor = new Color(redSlider.CurrentValue, greenSlider.CurrentValue, blueSlider.CurrentValue, alphaSlider?.CurrentValue ?? 1);
            ...
            if (needAlpha)
            {
                alphaSlider.OnChange += f => {
                    sample.BackColor = new Color(redSlider.CurrentValue, greenSlider.CurrentValue, blueSlider.CurrentValue, alphaSlider.CurrentValue);
                    changeAction(sample.BackColor);
                };
            }
            ...
            if (needAlpha) holder.Add(alphaSlider);
            colorHolder = sample;
            return holder;
        }
    }
