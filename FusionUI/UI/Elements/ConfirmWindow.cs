...
namespace FusionUI.UI.Elements
{
    class ConfirmWindow : Window {
        ...
        public ConfirmWindow(FrameProcessor ui, float x, float y, float w, float h, string text, Color backColor, bool drawCross = true)
            : this(x, y, w, h, text, backColor, drawCross) { }

        public ConfirmWindow(float x, float y, float w, float h, string text, Color backColor, bool drawCross = true)
            : base(x, y, w, h, text, backColor, drawCross)
        {
            Border = 1;
        }
        ...
        {
            var zoneText = new ScalableFrame(0, 0, 0, 0, "", Color.Zero)
            {
                Border = 1,
//                AutoHeight = true
            };

            textMessage = new RichTextBlock(leftOffsetX, 0, this.UnitWidth - 2 * leftOffsetX, minSizeText, message, Color.Zero, UIConfig.FontBase, 5)
            {
                Border = 1
            };
            ...
            this.Add(zoneText);
            var zoneButton = new ScalableFrame(0, 0, this.UnitWidth, UIConfig.UnitScenarioConfigLayersButtonHeight + leftOffsetY * 2, "", Color.Zero )
            {
                Border = 1
            };
            this.Add(zoneButton);
            var widthButton = (UIConfig.UnitPopupWindowWidth - leftOffsetX*2 - UIConfig.UnitBetweenButtonOffsetX)/2;
            OkButton = new Button(leftOffsetX, leftOffsetY,
            ...
            CancelButton = new Button(leftOffsetX + widthButton + UIConfig.UnitBetweenButtonOffsetX,
